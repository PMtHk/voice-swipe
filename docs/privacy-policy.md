...
- 마이크는 지원 페이지(Instagram Reels)에서 사용자가 명시적으로 켠 경우에만 작동합니다
...
| `host_permissions` (Instagram Reels) | 해당 페이지의 DOM 조작 (영상 전환, 볼륨 제어) |
...
Microphone access is used solely for voice command recognition on supported pages (Instagram Reels).
